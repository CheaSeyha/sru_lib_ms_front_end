...
const ModalDeleteCollege = ({ isModalVisible, handleCloseModal, fetchcollege, entry }) => {
    if (!entry) return null;
  // Function to handle radio button changes for dateOfBirth
  const handleSubmit = async (e) => {
    e.preventDefault();
    // try {
    axios.delete(`/college/${entry.collegeId}`).then(response => {
      toast.success(`បានបានលុបជំនាញ${entry.collegeName}ដោយជោគជ័យ!!!`, { style: { fontFamily: ' NotoSansKhmer-Regular, sans-serif' } });
      ...
      <Modal isVisible={isModalVisible} onClose={handleCloseModal}>
        <div className="container w-full h-full space-y-5">
          <div className="header-modal flex items-center justify-between">
            ...
            <label className='font-noto font-semibold text-lg'>លុបមហាវិទ្យាល័យ</label>
            ...
            <div className="container w-full h-full space-y-5">
              ...
              <div className='flex'>
                <div className="w-full mr-1">
                  <div className=" w-full font-noto text-xl">តើអ្នកពិតជាចង់លុបមហាវិទ្យាល័យ​ {entry.collegeName} មែនដែរឬទេ?:</div>
                </div>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4 content-center pt-5 pb-5">
              <BtnGredient type='submit' className="rounded-" color={'from-[#ff0000] to-[#fe8383]'} hover={'from-[#fe8383] to-[#ff0000]'}>
