...
const ModalDeleteMajor = ({ isModalVisible, handleCloseModal, fetchmajor, entry }) => {
    if (!entry) return null;
  // Function to handle radio button changes for dateOfBirth
  const handleSubmit = async (e) => {
    e.preventDefault();
    // try {
    axios.delete(`/major/${entry.majorId}`).then(response => {
      toast.success(`បានបានលុបជំនាញ${entry.majorName}ដោយជោគជ័យ!!!`, { style: { fontFamily: ' NotoSansKhmer-Regular, sans-serif' } });
            ...
            <div className="container w-full h-full space-y-5">
              ...
              <div className='flex'>
                <div className="w-full mr-1">
                  <div className=" w-full font-noto text-xl">តើអ្នកពិតជាចង់លុបជំនាញ​ {entry.majorName} មែនដែរឬទេ?:</div>
                </div>
              </div>
            </div>
