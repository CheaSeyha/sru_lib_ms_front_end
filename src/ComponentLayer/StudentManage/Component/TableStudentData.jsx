...
import { UserPlus, X, ArrowDownWideNarrow, School, Trash2, Pencil} from 'lucide-react';
...
function TableStudentData() {
    ...
    const fetchstudent = () => {
        axios.get('/student')
          .then(response => {
            ...
            setStudent(flatData);
            const uniquegen = [...new Set(response.data.map(stu => stu.generation))];
      ...
      const fetchmajor = () => {
        axios.get('/major')
          ...
          .catch(error => {
            console.error("There was an error fetching the major!", error);
          });
      };
      const fetchcollege = () => {
        axios.get('/college')
          .then(response => {
            setColege(response.data);
          })
          .catch(error => {
            console.error("There was an error fetching the college!", error);
          });
      };
      useEffect(() => {
        fetchstudent();
  ...
  const handleSubmitCollege = async (e) => {
    ...
    // try {
    axios.post('/college', submissionDataCollege, {
      headers: {
        'Content-Type': 'application/json',
      },
    }).then(response => {
      toast.success('បានបញ្ចូលមហាវិទ្យាល័យដោយជោគជ័យ!!!', { style: { fontFamily: ' NotoSansKhmer-Regular, sans-serif' } });
      setFormDataCollege({
        collegeId:'',
        collegeName:''
      })
  ...
  const handledelete = async () => {
    ...
    toast.success(`បានលុបសិស្សអត្តលេខ ${selectedRows[i].studentId} ដោយជោគជ័យ!!!`,{style:{fontFamily:' NotoSansKhmer-Regular, sans-serif'}});
}
    catch (error) {
        console.error(`Error deleting student with ID: ${selectedRows[i].studentId}`, error);
      }
    }
    //toast.success('បានលុបសិស្សដោយជោគជ័យ!!!',{style:{fontFamily:' NotoSansKhmer-Regular, sans-serif'}}); // Show success toast
        setSelectedRows([]);
        fetchstudent();
  };
        ...
        <>
            ...
            <div className='w-full h-full flex flex-col gap-5'>
                ...
                <div className="inline-block pr-2">
                  <BtnGredient1 className="w-full h-full" onClick={handledelete} color={'from-[#ff0000] to-[#E7FBFF]'} hover={'hover:from-[#E7FBFF] hover:to-[#ff0000]'}>
                  <Trash2 />
                ...
                <div className="inline-block pl-2 ">
                  <BtnGredient1 className="w-full h-full pr-5" onClick={() => setIsModalUpdate(true)} color={'from-[#1aff00] to-[#E7FBFF]'} hover={'hover:from-[#E7FBFF] hover:to-[#1aff00]'}>
                  <Pencil />
                    ...
                    {/* <div className='w-2/5 bg-slate-500'></div> */}
                ...
                <div className='table-container overflow-y-auto gap-5 flex-1 flex-col lg:flex-row md:flex-col flex w-full items-start'>
                    ...
                    <div className="table-container w-full h-[700px] md:h-full overflow-auto rounded-[10px] border border-primary shadow-xl">
                        ...
                        <table className="table tectav min-w-full divide-y divide-gray-200" onContextMenu={e => e.preventDefault()} onMouseUp={handleMouseUp}>
                            ...
                            <tr key={entry.studentId} onClick={()=>setSelectedRows([])} onMouseDown={(e) => handleMouseDown(entry, e)} onMouseOver={() => handleMouseOver(entry)} className={`hover:bg-primary ${selectedRows.some(row => row.studentId === entry.studentId) ? 'bg-primary' : ''} text-sm cursor-pointer active:bg-primary`}>
                                <td>{index + 1}</td>
                        ...
                        <div className="table-container scrollbar-hide overflow-auto border border-primary shadow-xl rounded-[10px]">
                            <table className="table">
                                ...
                                {major.map((entry, index) => (
                                    <tr key={entry.majorId} onClick={() => handleDeleteMajor(entry)} className=' hover:bg-primary cursor-pointer active:bg-primary'>
                                        <td>{index+1}</td>
                                        <td>{entry.majorId}</td>
                                        <td>{entry.majorName}</td>
                        ...
                        <div className="table-container scrollbar-hide overflow-auto rounded-[10px] border border-primary shadow-xl">
                            <table className="table">
                                ...
                                {college.map((entry, index) => (
                                    <tr key={entry.collegeId} onClick={() => handleDeleteCollege(entry)} className='text-[15px] hover:bg-primary cursor-pointer active:bg-primary'>
                                        <td>{index+1}</td>
                                        <td>{entry.collegeId}</td>
            ...
            <ModalDeleteCollege isModalVisible={isModalDeleteCollege} handleCloseModal={handleCloseModal} fetchcollege={fetchcollege} entry={deleteEntry}/>
            <ModalDeleteMajor isModalVisible={isModalDeleteMajor} handleCloseModal={handleCloseModal} fetchmajor={fetchmajor} entry={deleteEntry}/>
            <ModalUpdateStudent isModalVisible={isModalUpdate} handleCloseModal={handleCloseModalUpdate}fetchstudent={fetchstudent} rowSelected={selectedRows} setRowSelected={setSelectedRows}/>
            <ModalAddStudent isModalVisible={isShowModal} handleCloseModal={handleCloseModal} fetchStudent={fetchstudent}/>
            <ModalAddMajor isModalVisible={isShowModalMajor} handleCloseModal={handleCloseModalMajor} fetchmajor={fetchmajor}/>
            {/* ModalMajor */}
            ...
            <div className="container w-full h-full space-y-5">
              ...
              <div className='flex mt-5'>
                <div className="w-2/5 mr-1">
                  ...
                  <input type="text" className="input input-bordered  w-full bg-secondary font-noto" name="collegeId" value={formDataCollege.collegeId} onChange={handleChangeCollge} required />
                </div>
                <div className="mr-1 w-full">
                  <div className=" font-noto font-semibold">មហាវិទ្យាល័យ :</div>
                  <input type="text" className="input input-bordered  w-full bg-secondary font-noto" name="collegeName" value={formDataCollege.collegeName} onChange={handleChangeCollge} required />
                </div>
              </div>
            </div>
