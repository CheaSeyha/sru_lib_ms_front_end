...
export default function TableStuEntry({
  ...
  const [studentIDErrorMsx, setStudentIDErrorMsx] = useState(
  ...
  const { handleCheckScanEntryExit, handleSaveEntry, handleSearchStudent } =
    useCheckStudentEntryExit();

  // Use WebSocket data if available, otherwise fall back to API data from the hook
  // Reverse the array to show the most recent entries at the top
  const displayData = [...(wsEntries || studetnEntryData || [])].reverse();

  // Automatically refresh table data when a WebSocket signal arrives
  // and we don't have the full entries list in the WS payload
  useEffect(() => {
    if (wsRefreshTrigger > 0 && !wsEntries) {
      console.log("🔄 WS trigger: Fetching fresh table data...");
      fetchRecentEntryData();
    }
  }, [wsRefreshTrigger, wsEntries]);
  ...
  const handleEntry = async () => {
    if (!checkRadio) {
      const checks = [
        {
          condition: guestName.trim() === "",
          action: () => setGuestNameError(true),
          reset: () => setGuestNameError(false),
        },
        {
          condition: gender.trim() === "",
          action: () => setGenderError(true),
          reset: () => setGenderError(false),
        },
        {
          condition: position.trim() === "",
          action: () => setPositionError(true),
          reset: () => setPositionError(false),
        },
        {
          condition: checkPurpose === "",
          action: () => {
        ...
        <div className="container w-full h-full space-y-5">
          ...
          {checkRadio ? (
            <div className="form-entry grid gap-2">
              ...
              {studentIDError && (
                <p
                  className={`text-sm mt-1 flex items-center ${studentIDErrorColor ? "text-red-500 " : "text-green-400"}`}
                >
                  {studentIDErrorColor ? <ShieldAlert /> : <ShieldCheck />}
                  {studentIDErrorMsx}{" "}
                </p>
              )}
          ...
          <div className="check-purpose text-accent mt-5">
            ...
            <div className="container-check-purpose grid grid-cols-2 sm:grid-cols-4 gap-5 pt-5">
              <div className="check-purpose flex items-center space-x-2">
                ...
                />
                <label htmlFor="Reading" className="label-text text-[#32E2FF]">
                  Read Book
                </label>
              </div>
              <div className="check-purpose flex items-center space-x-2">
              ...
              <div className="check-purpose flex items-center space-x-2">
                <input
                  ...
                  onChange={handleCheckPurpose}
                />
                <label htmlFor="Use_PC" className="label-text text-[#32E2FF]">
                  Use PC
                </label>
              </div>
              <div className="check-purpose flex items-center space-x-2">
                <input
                  ...
                  onChange={handleCheckPurpose}
                />
                <label htmlFor="Other" className="label-text text-[#32E2FF]">
                  Other
                </label>
              </div>
            </div>
