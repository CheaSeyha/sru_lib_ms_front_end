...
const ModalUpdateStudent = ({ isModalVisible, handleCloseModal,fetchstudent, rowSelected,setRowSelected }) => {
...
useEffect(() => {
    if (rowSelected && rowSelected.length > 0) {
        ...
        setFormData({
            ...
            majorId: majordata.find(row => row.majorName === selectedStudent.majorName).majorId || '',
            degreeLevelId: degreedata.find(row => row.degreeLevel === selectedStudent.degreeLevel).degreeLevelId || ''
        });
    }
}, [rowSelected]);
...
const handleSubmit = async (e) => {
    ...
    try {
        ...
        if (updateResponse.status === 202 || updateResponse.status === 200) {
            toast.success('អ្នកបានកែរប្រែព័ត៌មានសិស្សបានដោយជោគជ័យ!!!', { style: { fontFamily: ' NotoSansKhmer-Regular, sans-serif' } });
            fetchstudent();
            setRowSelected([]);
        } else {
  ...
  return (
      ...
      <Modal isVisible={isModalVisible} onClose={handleCloseModal}>
        <div className="container w-full h-full space-y-5">
          <div className="header-modal flex items-center justify-between">
            ...
            <div className="flex justify-end mt-0 w-1/3">
            </div>
            ...
            <div className="container w-full h-full space-y-5">
              ...
              <div className='flex'>
                <div className="w-full mt-5 mr-1">
                  ...
                  <input type="text" className="input input-bordered  w-full bg-secondary font-noto" name="studentId" value={formData.studentId} onChange={handleChange} readOnly />
                </div>
                <div className="w-full mt-5 ml-1">
                  <div className=" w-full font-noto font-semibold">ឈ្មោះ :</div>
                  <input type="text" className="input input-bordered  w-full bg-secondary font-noto" name="studentName" value={formData.studentName} onChange={handleChange} />
                </div>
              </div>
              <div className='flex'>
    ...
    <option value="ST">Other</option> */}
                ...
                <div className="w-1/4 ml-1">
                  ...
                  <input type="number" className="input input-bordered  w-full bg-secondary font-noto" name="generation" value={formData.generation} onChange={handleChange} />
                </div>
              </div>
            </div>
