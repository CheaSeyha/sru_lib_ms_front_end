...
const ListOfBorrow = () => {
    ...
    axios
      ...
      .then((response) => {
        const filteredBooks = response.data.filter(
          (borrow) => borrow.isBringBack === false,
        );
  ...
  const filteredData = borrow.filter(
    (item) =>
      (item.studentId
        .toString()
        .toLowerCase()
      ...
      (searchDate === null ||
        isEqual(
          parse(item.giveBackDate, "yyyy-MM-dd", new Date()),
          searchDate,
        )),
  );
  ...
  return (
    <>
        ...
        <div className="w-full flex flex-col-reverse xl:flex-row sm:flex-col-reverse m-0">
          <div className="flex w-full">
            <div className="inline-block h-full pr-5 w-full">
              <label
                htmlFor=""
                className="input input-bordered rounded-[50px] w-full md:w-full flex items-center bg-base-100 p-2 h-full gap-2"
              >
                <input
                  type="text"
        ...
        <div className="table-container overflow-y-auto flex-1 w-full grid items-start scrollbar-hide">
          <table className="table tectav min-w-full divide-y divide-gray-200">
            <thead className="text-accent">
              <tr>
                <th className="sticky top-0 text-left text-sm font-semibold bg-secondary">
                  ល.រ
                </th>
                <th className="sticky top-0 text-left text-sm font-semibold bg-secondary">
                  អត្តលេខនិស្សិត
      ...
      >
        ...
        <AlertWithSound
          newData={
            latestBorrowId.length > 0 &&
            latestBorrowId[latestBorrowId.length - 1]
          }
        />
      </div>
      <ModalBorrow
        ...
        setIsModalLstVisible={setIsModalLstVisible}
