...
const ModalBorrow = ({
  ...
  const handleContinue = () => {
    axios
      .put("/borrow/extend-borrow", null, {
        params: {
          id: entry.borrowId,
        },
      })
      .then((response) => {
        toast.success("បានបន្តការខ្ចីដោយជោយជ័យ!", {
          style: { fontFamily: " NotoSansKhmer-Regular, sans-serif" },
        }); // Show success toast
      })
      .catch((error) => {
    ...
    <>
      <Modal isVisible={isModalVisible} onClose={closeModal}>
        <div className="container w-full h-full space-y-5 font-noto">
          ...
          <div className="w-full">
            <div className="inline-block w-1/3 font-bold">
              កាលបរិច្ឆេទខ្ចី :
            </div>
            <div className="inline-block">
              <p>{entry.borrowDate}</p>
            </div>
          </div>
          <div className="w-full">
            <div className="inline-block w-1/3 font-bold">
